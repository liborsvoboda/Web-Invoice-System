...
if ($part[0]<>"customer"){$check = mysql_query("SELECT cash_no,".securesql($part[0])." FROM cash_header WHERE ".securesql($part[0])." like '".securesql($part[1])."%' order by ".securesql($part[0])." DESC");}
else                     {$check = mysql_query("SELECT cash_no,".securesql($part[0])." FROM cash_header WHERE ".securesql($part[0])." like '%".securesql($part[1])."%' ");}
...
document.getElementById('cash_search').innerHTML = '<? echo @$status; ?>';
<?
