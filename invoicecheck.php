...
if ($part[0]<>"customer"){$check = mysql_query("SELECT invoice_no,".securesql($part[0])." FROM invoices_header WHERE ".securesql($part[0])." like '".securesql($part[1])."%' order by ".securesql($part[0])." DESC");}
else                     {$check = mysql_query("SELECT invoice_no,".securesql($part[0])." FROM invoices_header WHERE ".securesql($part[0])." like '%".securesql($part[1])."%' ");}
...
document.getElementById('inv_search').innerHTML = '<? echo @$status; ?>';
<?
